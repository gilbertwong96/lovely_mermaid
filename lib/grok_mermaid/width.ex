defmodule GrokMermaid.Width do
  ...
  @doc "Width of one code point; the table covers the whole code point space."
  ...
  defp do_lookup(runs, cp, lo, hi) when lo <= hi do
    ...
    cond do
      ...
      cp > h -> do_lookup(runs, cp, mid + 1, hi)
      true -> w
    end
  end

  defp do_lookup(_runs, _cp, _lo, _hi), do: 1
  ...
  the base. A variation selector requesting emoji presentation forces two
  columns, as does a regional indicator pair (a flag).
  """
  @spec cluster_width(String.t()) :: non_neg_integer()
  def cluster_width(cluster) do
    {w, vs16, regional} =
      cluster
      |> String.to_charlist()
      |> Enum.reduce({0, false, 0}, fn cp, {w, vs16, regional} ->
        vs16 = vs16 or cp == @vs16
        regional = regional + if(regional_indicator?(cp), do: 1, else: 0)
        {max(w, code_point_width(cp)), vs16, regional}
      end)

    if vs16 or regional >= 2, do: 2, else: w
  end

  @doc "Iterate grapheme clusters, so no loop can split one."
  @spec clusters(String.t()) :: [String.t()]
  def clusters(s), do: String.graphemes(s)

  @doc "Clusters paired with their display width."
  @spec measured(String.t()) :: [{String.t(), non_neg_integer()}]
  def measured(s) do
    s |> String.graphemes() |> Enum.map(&{&1, cluster_width(&1)})
  end

  @doc "Display columns of a string."
  @spec string_width(String.t()) :: non_neg_integer()
  def string_width(s) do
    s |> String.graphemes() |> Enum.reduce(0, fn g, acc -> acc + cluster_width(g) end)
  end
end
