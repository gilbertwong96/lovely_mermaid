defmodule LovelyMermaid.CoverageTest do
  ...
  alias LovelyMermaid
  alias LovelyMermaid.{Canvas, Labels, SourceBox}

  # ---------------------------------------------------------------- entry

  test "render rejects non-string input" do
    assert LovelyMermaid.render(123) == nil
    assert LovelyMermaid.render(nil) == nil
  end
  ...
  test "bare headers render nothing" do
    for header <- [
      ...
      assert LovelyMermaid.render(header) == nil, header
      assert LovelyMermaid.diagram_kind(header) != nil, header
    end
  end
  ...
  test "retry fails too when the salvage is unreadable" do
    assert LovelyMermaid.render("stateDiagram-v2\n  garbage one\n  garbage two") == nil
  end
  ...
  test "state direction RL and BT orient the layout" do
    rl = LovelyMermaid.render("stateDiagram-v2\n  direction RL\n  [*] --> A\n  A --> [*]")
    assert rl != nil

    bt = LovelyMermaid.render("stateDiagram-v2\n  direction BT\n  [*] --> A\n  A --> [*]")
    assert bt != nil
  end
  ...
  test "class grammar rejects unreadable statements" do
    assert LovelyMermaid.render("classDiagram\n  ???") == nil
    assert LovelyMermaid.render("classDiagram\n  bad line") == nil
  end

  test "er grammar rejects unreadable statements" do
    assert LovelyMermaid.render("erDiagram\n  B --") == nil
  end

  test "sequence grammar rejects unreadable statements" do
    assert LovelyMermaid.render("sequenceDiagram\n  ??? one\n  ??? two") == nil
  end
  ...
  test "over-capacity graphs are refused" do
    src = "flowchart TD\n" <> Enum.map_join(1..130, "\n", &"n#{&1} --> n#{&1 + 1}")
    assert LovelyMermaid.render(src) == nil
  end
  ...
  test "dotted and thick edges render distinct glyphs" do
    art = LovelyMermaid.render("flowchart LR\n  A -.-> B\n  C ==> D")
    assert art != nil
    assert Enum.join(art.plain, "\n") =~ "╌"
  end
  ...
  test "a note registers participants not otherwise declared" do
    art = LovelyMermaid.render("sequenceDiagram\n  Note over A,B: hello")
    assert art != nil
    joined = Enum.join(art.plain, "\n")
    assert joined =~ "│ A │"
    assert joined =~ "│ B │"
    ...
    left = LovelyMermaid.render("sequenceDiagram\n  Note left of A: hi")
    assert left != nil
...
defmodule LovelyMermaid.GrammarTableTest do
  ...
  test "state declaration forms" do
    ...
    assert LovelyMermaid.render(quoted) != nil
    ...
    assert LovelyMermaid.render(choice) != nil
    ...
    assert LovelyMermaid.render(braced) != nil
  end
  ...
  test "er cardinality operators all render" do
    for {op, card_l, card_r} <- [
      ...
      art = LovelyMermaid.render("erDiagram\n  A #{op} B : has")
...
defmodule LovelyMermaid.LayoutVariantTest do
  ...
  test "self loops with labels reserve width in both directions" do
    td = LovelyMermaid.render("flowchart TD\n  A --> B\n  B -- loops --> B")
    assert td != nil
    assert Enum.join(td.plain, "\n") =~ "loops"

    lr = LovelyMermaid.render("flowchart LR\n  A --> B\n  B -- loops --> B")
    assert lr != nil
  ...
  test "class braces outside a declaration are ignored" do
    art = LovelyMermaid.render("classDiagram\n  class A\n  }")
    assert art != nil
    assert Enum.join(art.plain, "\n") =~ "A"
  end
